...
function preload(){
  bigBody = loadImage('images/body2.png');
...
function draw(){
  ...
  for (var q = 0; q < height; q++) {
    for (var p = 0; p < width; p++) {
      var index = ( p + q * width) * 4;
      pixels[index+0] = pixels[index+0] + red;
      pixels[index+1] = pixels[index+1] + green;
      pixels[index+2] = pixels[index+2]+ blue;
      // pixels[index+3] = 255;
    }
  }  
  updatePixels();
  ...
  if (playing == false) {
    textSize(16);
    fill(255);
    text('Click anywhere to start the music. Scroll down and right to discover the world', 50, 50);
  }  
}
...
function showRGB(r, g, b){
  ...
  // if (frameCounter == 100){
    textSize(32);
    fill(255);
    text('R: ' + parseInt(r) + '/ G: ' + parseInt(g) + '/B: ' + parseInt(b), 30, 30);
    frameCounter = 0;
  // }
}
